  ...
  try {
    ...
    if (response.status === 401) {
      if (redirectOnUnauthorized) {
        ...
        throw new ApiError('Unauthorized - Redirecting to login', 401, 'Unauthorized');
      } else {
...
export const getTaskCounter = async (type: 'indexer' | 'checker'): Promise<{ counter: number; type: string }> => {
...
export const emailChangeApi = {
  ...
  validateEmail: async (email: string): Promise<any> => {
    const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/user/email/validate?email=${encodeURIComponent(email)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  initiateChange: async (newEmail: string): Promise<any> => {
    const response = await postWithAuth('/user/email/initiate-change', { newEmail });
    return response;
  },

  // Verify email change with 2FA
  verifyWith2FA: async (twoFactorToken: string): Promise<any> => {
    const response = await postWithAuth('/user/email/verify-2fa', { twoFactorToken });
    return response;
  },
