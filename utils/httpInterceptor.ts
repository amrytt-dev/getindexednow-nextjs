...
const interceptedFetch = async (input: RequestInfo, init?: RequestInit): Promise<Response> => {
  ...
  const isApiRequest = url.includes(import.meta.env.VITE_API_URL || 'localhost:3001') || 
                      url.startsWith('/api') ||
                      url.includes('/api/');
