...
import { ChevronDown, ChevronUp, Eye, Download, Zap, Calendar, CheckCircle, RefreshCw, FileText, BarChart3, Link, TrendingUp, CreditCard, Clock, ChevronRight, ActivityIcon } from 'lucide-react';
...
export const TasksList = ({ taskType, selectedTaskId, onCreateTask }: TasksListProps) => {
  const safeType: 'indexer' | 'checker' = taskType === 'checker' ? 'checker' : 'indexer';
  ...
  const { data: tasksData, isLoading, error, refetch } = useTasksList(
    safeType, 
    user?.id, 
    {
  ...
  const pendingTasksForType = pendingTasks.filter(task => task.type === safeType);
  ...
  const handleDownloadReport = async (task: any, event: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/reports/download?format=csv&taskId=${task.id}`, {
        method: 'GET',
        headers: {
  ...
  if (userLoading) {
    return (
      <div className="w-full">
        <TasksListHeader taskType={taskType} onSearch={setSearchQuery}
                         onFilterChange={(val) => {
                           setActiveFilter(val);
                         }} />
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            {filteredTasks.map((task) => {
                     ...
                     const isPendingTask = task.id && task.id.startsWith('pending-');
                     const progressPercentage = isPendingTask ? 0 : (typeof task.size === 'number' && typeof task.indexed_count === 'number' && task.size > 0
                       ? Math.min(Math.round((task.indexed_count / task.size) * 100), 100)
                       : 0);
              ...
              return (
                  ...
                  ref={task.id === selectedTaskId ? selectedTaskRef : null}
                          className={`border-b border-[#dadce0] transition-colors ${
                            task.id === selectedTaskId ? 'ring-2 ring-[#4285f4] bg-[#e8f0fe]' : 
                            isPendingTask ? 'bg-gray-50 opacity-75' : 'hover:bg-[#f8f9fa]'
                          }`}
                        >
                          {/* Task Type Icon */}
                          <TableCell className="w-12">
                            <span className="text-xl">
                              {isPendingTask ? '⏳' : (task.speedyType && task.speedyType.includes('indexer') ? '🚀' : '🔍')}
                            </span>
                          </TableCell>
                          ...
                          {/* Status */}
                          <TableCell>
                            <Badge className={`rounded-full px-3 py-1 text-sm font-medium ${getStatusColor(computedStatus)}`}>
                              {computedStatus}
                            </Badge>
                          </TableCell>
                          ...
                          <TableCell>
                            <div className="flex flex-col">
                              <span className="font-medium text-[#202124]">
                                {isPendingTask ? `${task.urls?.length || 0}` : `${task.indexed_count?.toLocaleString() || '0'}/${task.size?.toLocaleString() || '0'}`}
                              </span>
                              <span className="text-xs text-[#5f6368]">
                                {isPendingTask ? 'pending' : `${task.processed_count?.toLocaleString() || '0'} processed`}
                              </span>
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="w-32">
                            <div className="space-y-1">
                              ...
                              />
                              <span className="text-xs text-[#5f6368]">{progressPercentage}%</span>
                            </div>
                          </TableCell>
                          ...
                          <TableCell>
                            <Tooltip delayDuration={300}>
                              <TooltipTrigger asChild>
                                <div className="cursor-help">
                                  <span className="font-medium text-[#fbbc04]">
                                    {isPendingTask ? `${task.urls?.length || 0} (pending)` : (() => {
                              ...
                              <TooltipContent side="top" className="bg-white border border-[#dadce0] shadow-lg p-3 rounded-lg">
                                <div className="space-y-1 text-sm">
                                  <p>Total credits for task: {(task.size || 0) * 1}</p>
                                  <p>Credits on hold: {task.heldCredits?.toLocaleString() || '0'}</p>
                                  <p>Credits used: {task.creditUsage?.toLocaleString() || '0'}</p>
                          ...
                          <TableCell>
                            <div className="flex items-center space-x-2 text-sm text-[#5f6368]">
                              ...
                              <span>
                                {isPendingTask ? 'Just now' : (task.created_at && !isNaN(new Date(task.created_at).getTime()) 
                           ...
                           <TableCell className="w-32">
                             <div className="flex items-center space-x-2">
                               ...
                               <div className="w-8 h-8">
                                 {!isPendingTask && computedStatus === 'Completed' && (task.type === 'indexer' || task.type === 'google/indexer' || (task.speedyType && task.speedyType === 'fastindex/indexer') || (task.speedyType && task.speedyType.includes('indexer'))) ? (
                                   <Tooltip delayDuration={300}>
                                     <TooltipTrigger asChild>
                                       <Button 
                                         variant="outline" 
                                         size="sm"
                                         onClick={(e) => handleDownloadReport(task, e)}
                             ...
                             >
                               <td colSpan={8} className="p-0">
                                 <div className="bg-[#f8f9fa] border-t border-[#dadce0] p-4">
                                                                    <TaskLinksTable
                                     taskId={task.id || ''}
                                     type={(task.type === 'checker' || task.type === 'google/checker' || (task.speedyType && task.speedyType.includes('checker'))) ? 'checker' : 'indexer'}
            ...
            {totalPages > 1 && (
              <Pagination className="mt-6">
                <PaginationContent>
                  <PaginationItem>
                    <PaginationPrevious 
                      className={`rounded-full border-[#dadce0] hover:bg-[#f1f3f4] hover:text-[#4285f4] ${currentPage === 1 ? 'pointer-events-none opacity-50' : 'cursor-pointer'}`}
                    ...
                    return pages.map((page, index) => (
                      <PaginationItem key={index}>
                        {page === '...' ? (
                          ...
                          <PaginationLink 
                            className={`rounded-full border-[#dadce0] hover:bg-[#f1f3f4] cursor-pointer ${currentPage === page ? 'bg-[#4285f4] text-white' : ''}`}
                  ...
                  <PaginationItem>
                    <PaginationNext 
                      className={`rounded-full border-[#dadce0] hover:bg-[#f1f3f4] hover:text-[#4285f4] ${currentPage === totalPages ? 'pointer-events-none opacity-50' : 'cursor-pointer'}`}
                      onClick={handleNext}
                    />
                  </PaginationItem>
                </PaginationContent>
