...
const extractValidUrls = (input: string): { urls: string[], correctedInput: string, hasErrors: boolean, invalidLines: string[] } => {
...
export const SubmitTaskForm = ({ onTaskSubmitted }: SubmitTaskFormProps) => {
  const { register, handleSubmit, control, setValue, watch, formState: { errors, isSubmitting } } = useForm<TaskFormValues>({
    defaultValues: {
      title: '',
      urls: '',
      type: 'indexer',
    ...
    const generateTaskName = async () => {
      if (!isNameManuallyChanged) {
        try {
          ...
          const taskName = `${type === 'indexer' ? 'Indexer' : 'Checker'} Task #${randomId}`;
  ...
  const urlStats = useMemo(() => {
    ...
    const { urls: validUrls, hasErrors, invalidLines } = extractValidUrls(urlsValue);
  ...
  const isSubmissionDisabled = useMemo(() => {
    if (isOutOfCredits || creditsLoading || urlValidationError || isSubmittingTask) {
      return true;
    }
    ...
    if (creditsInfo && urlStats.requiredCredits > creditsInfo.creditsAvailable) {
      return true;
    }

    return false;
  }, [isOutOfCredits, creditsLoading, urlValidationError, urlStats, useVip, creditsInfo, isSubmittingTask]);

  // Handle URL input changes for real-time validation
  const handleUrlInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    const input = e.target.value;
    const { urls, correctedInput, hasErrors, invalidLines } = extractValidUrls(input);

    console.log('URL validation:', { input, urls, correctedInput, hasErrors, invalidLines });

    if (hasErrors) {
      if (invalidLines.length > 0) {
        const invalidExamples = invalidLines.slice(0, 3).join(', ');
        setUrlValidationError(`Invalid URLs detected: "${invalidExamples}"${invalidLines.length > 3 ? ' and more...' : ''}. All URLs must start with http:// or https:// protocol. Use "Clean URLs" to remove invalid parts.`);
      } else {
        setUrlValidationError('Some URLs appear to be malformed. Click "Clean URLs" to auto-correct them.');
      }
  ...
  const applyUrlCorrections = () => {
    ...
    toast({
      ...
      description: 'Invalid URLs have been removed and formatting has been corrected.',
    });
  };
  ...
  const cleanUrls = () => {
    ...
    if (cleanedUrls.length === 0) {
      toast({
        ...
        description: 'No valid URLs to keep. Please add URLs with http:// or https:// protocol.',
        variant: 'destructive',
      });
      return;
    }
    ...
    toast({
      title: 'URLs cleaned',
      description: `Removed invalid URLs. Kept ${cleanedUrls.split('\n').filter(line => line.trim() !== '').length} valid URL(s).`,
    });
  };
  ...
  const onSubmit = async (values: TaskFormValues) => {
    ...
    const { urls: validUrls, hasErrors, invalidLines } = extractValidUrls(values.urls);
    ...
    if (validUrls.length === 0) {
      toast({
        title: 'No valid URLs',
        description: 'Please enter at least one valid URL starting with http:// or https:// protocol.',
        variant: 'destructive',
      });
      setIsSubmittingTask(false);
      return;
    ...
    const endpoint = (values.type === 'indexer') ? '/vip/create' : '/proxy/speedyindex';
    ...
    if (values.type === 'indexer') {
      ...
      toast({
        ...
        description: 'Task created successfully and queued for processing. You can continue with other tasks while this processes in the background.',
      });
    } else {
      ...
      .then((result) => {
        if (result.code === 0) {
          ...
          if (values.type === 'indexer' && result.task?.id) {
            console.log('🚀 Task ID received for Indexer task:', result.task.id);
          } else {
            console.log('❌ Not showing progress - Type:', values.type, 'Task ID:', result.task?.id);
          }
          ...
          const errorMessage = result.message || result.error || 'Failed to submit task';
      ...
      {isOutOfCredits ? (
        <div className="flex flex-col items-center space-y-4 p-6 bg-gradient-to-r from-warning/10 to-warning/5 text-center">
          ...
          <div className="text-foreground/80 font-medium max-w-md">
            {creditsInfo?.heldCredits && creditsInfo.heldCredits > 0 ? (
              <>
                You have used all available credits ({creditsInfo.usedCredits} used, {creditsInfo.heldCredits} on hold).
                Please wait for your tasks to complete and credits to be released, or upgrade your plan.
              </>
            ) : (
              'You have reached your monthly credit limit. Please upgrade your plan or wait for the next period.'
            )}
          </div>
          <Link to="/plans-billing">
            <Button
              variant="hero"
              size="lg"
              className="mt-2"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label className="text-sm text-gray-700 uppercase ">🔧 Task Type</Label>
            <Controller
                name="type"
                control={control}
                render={({ field }) => (
          ...
          data-[state=on]:bg-gradient-to-r data-[state=on]:from-secondary data-[state=on]:to-primary data-[state=on]:text-white"
            ...
            <p className="text-xs text-muted-foreground">
              {type === 'indexer' ? 'Submit URLs for Google indexing' : 'Check current indexing status'} • Uses unified credits
            </p>
          </div>

          {/* Task Title */}
          <div className="space-y-2">
            <Label htmlFor="title" className="text-sm  text-gray-700 uppercase ">
              📝 Task Title {!isNameManuallyChanged && <span className="text-xs text-blue-600 ">(Auto-generated)</span>}
            </Label>
            <Input
              id="title"
          ...
          <div className="space-y-2">
            ...
            {errors.urls && <p className="text-red-500 text-xs mt-1">{errors.urls.message}</p>}
            {creditsInfo && (
                    <div className="mt-3 pt-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-blue-700">
                          Available Credits: <span className="font-semibold">{creditsInfo.creditsAvailable}</span>
                        </span>
                  ...
                  <div className="flex justify-between flex-wrap text-sm">
                    <div className="text-center">
                      <div className="font-semibold text-blue-700  ">
                          <span className="text-xs text-blue-600 ">Total URLs:</span> {urlStats.totalUrls}
                      </div>
                      
                    </div>
                    <div className="text-center">
                      <div className="font-semibold text-green-700 ">
                        <span className="text-xs text-green-600 ">Unique URLs:</span> {urlStats.uniqueUrls}
                      </div>
                    ...
                    {urlStats.duplicateCount > 0 && (
                      <div className="text-center">
                        <div className="font-semibold text-orange-700 ">
                          <span className="text-xs text-orange-600 ">Duplicates:</span> {urlStats.duplicateCount}
                        </div>
                    ...
                    <div className="text-center">
                      <div className="font-semibold text-purple-700 ">
                          <span className="text-xs text-purple-600 ">Credits Needed:</span> {urlStats.requiredCredits}
                      </div>
            ...
            <div className="text-xs text-gray-500  space-y-1">
              ...
              <p>💡 URLs must start with http:// or https:// • Use "Fix URLs" for corrections</p>
            </div>
          </div>
          ...
          {isSubmissionDisabled && (
            <div className="text-center text-sm text-gray-500 ">
              {urlStats.uniqueUrls === 0 && 'Please enter at least one valid URL'}
              {urlStats.uniqueUrls > 10000 && 'Maximum 10,000 URLs allowed per task'}
              {/* No VIP limit now */}
              {creditsInfo && urlStats.requiredCredits > creditsInfo.creditsAvailable && 
