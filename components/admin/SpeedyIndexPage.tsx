...
export default function SpeedyIndexPage() {
  ...
  const [loading, setLoading] = useState<{ status: boolean; full: boolean }>({ status: false, full: false });
  ...
  const handleFetch = async (type: 'status' | 'full') => {
    ...
    setLoading((s) => ({ ...s, [type === 'status' ? 'status' : 'full']: true }));
    try {
      ...
      const body = type === 'status'
        ? { task_ids: statusTaskIds.split(/\s|,|\n/).map(t => t.trim()).filter(Boolean) }
  ...
  const indexedRows = useMemo(() => {
    ...
    const raw = res.indexed_links ?? res.indexed ?? src.indexed_links ?? src.indexed ?? [];
    const arr = Array.isArray(raw) ? raw : [];
    return arr.map((item: any) => {
      if (typeof item === 'string') return { url: item, title: null };
      return { url: item.url ?? '-', title: item.title ?? item.page_title ?? null };
    });
  }, [reportResult]);
  ...
  const unindexedRows = useMemo(() => {
    const src = reportResult || {};
    const res = src.result || {};
    const raw = res.unindexed_links ?? res.unindexed ?? src.unindexed_links ?? src.unindexed ?? [];
    const arr = Array.isArray(raw) ? raw : [];
    return arr.map((item: any) => {
      if (typeof item === 'string') return { url: item, error_code: null };
      return { url: item.url ?? '-', error_code: item.error_code ?? item.error ?? null };
    });
  }, [reportResult]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">Checker and Indexer tools for Google</p>
      </div>
        ...
        <TabsContent value="checker" className="space-y-6">
          <Card className="rounded-xl shadow-sm">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={() => handleFetch('status')} disabled={loading.status}>
                  {loading.status && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Fetch Status
                </Button>
                <Button variant="outline" onClick={() => setStatusTaskIds('')}>Clear</Button>
              </div>
              {error && (
                <div className="flex items-center text-red-600 text-sm"><AlertCircle className="h-4 w-4 mr-2" />{error}</div>
              )}
              {statusResult && (
                <div className="space-y-3">
                  <details className="bg-gray-50 rounded-md p-3">
                    <summary className="cursor-pointer font-medium">Raw JSON</summary>
                    <pre className="mt-2 text-xs overflow-auto max-h-80">{jsonPreview(statusResult)}</pre>
                  </details>
                </div>
              )}
            </CardContent>
          ...
          <Card className="rounded-xl shadow-sm">
            ...
            <CardContent className="space-y-4">
              ...
              />
              <div className="flex items-center gap-2">
                <Button onClick={() => handleFetch('full')} disabled={loading.full}>
                  {loading.full && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Fetch Report
                </Button>
                <Button variant="outline" onClick={() => setFullReportTaskId('')}>Clear</Button>
              </div>
              {error && (
                <div className="flex items-center text-red-600 text-sm"><AlertCircle className="h-4 w-4 mr-2" />{error}</div>
              )}
              {reportResult && (
                <div className="space-y-4">
                  <details className="bg-gray-50 rounded-md p-3">
                    <summary className="cursor-pointer font-medium">Raw JSON</summary>
                    <pre className="mt-2 text-xs overflow-auto max-h-80">{jsonPreview(reportResult)}</pre>
                  </details>
                            ...
                            <tbody>
                              {indexedRows.map((row: any, i: number) => (
                                <tr key={i} className="border-t">
                                  <td className="py-2 pr-4 break-all text-blue-700">{row.url}</td>
                                  <td className="py-2">{row.title || '-'}</td>
                                </tr>
                              ))}
                              {indexedRows.length === 0 && (
                                <tr><td className="py-3 text-gray-500" colSpan={2}>No indexed links</td></tr>
                              )}
                            </tbody>
                          </table>
                        </div>
                          ...
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {unindexedRows.map((row: any, i: number) => (
                                <tr key={i} className="border-t">
                                  <td className="py-2 pr-4 break-all text-blue-700">{row.url}</td>
                                  <td className="py-2">{row.error_code || '-'}</td>
                                </tr>
                              ))}
                              {unindexedRows.length === 0 && (
                                <tr><td className="py-3 text-gray-500" colSpan={2}>No unindexed links</td></tr>
                              )}
                            </tbody>
                          </table>
                        </div>
