...
import { CalendarIcon, Users, CreditCard, DollarSign, TrendingUp, Filter } from 'lucide-react';
...
export const AdminDashboard = () => {
  ...
  const [subscribedUsersFilter, setSubscribedUsersFilter] = useState<string>('month');
  const [totalRevenueFilter, setTotalRevenueFilter] = useState<string>('month');
  const [conversionRateFilter, setConversionRateFilter] = useState<string>('month');

  // Helper function to build query parameters
  const buildQueryParams = (filter: string, dateRange: { from: Date | undefined; to: Date | undefined }) => {
  ...
  const getEffectiveFilter = (localFilter: string) => {
    ...
    if (globalFilter === 'custom' && customDateRange.from && customDateRange.to) {
      return 'custom';
    }
  ...
  const { data: dashboardData, isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<DashboardStats> => {
      ...
      const response = await fetch(`${backendBaseUrl}/api/admin/dashboard?${params.toString()}`, {
        headers: {
  ...
  const { data: totalUsersData } = useQuery({
    queryKey: ['admin-total-users', totalUsersFilter, globalFilter, customDateRange],
    queryFn: async () => {
      const backendBaseUrl = import.meta.env.VITE_API_URL || '';
  ...
  const { data: subscribedUsersData } = useQuery({
    queryKey: ['admin-subscribed-users', subscribedUsersFilter, globalFilter, customDateRange],
    queryFn: async () => {
      const backendBaseUrl = import.meta.env.VITE_API_URL || '';
  ...
  const { data: totalRevenueData } = useQuery({
    queryKey: ['admin-total-revenue', totalRevenueFilter, globalFilter, customDateRange],
    queryFn: async () => {
      const backendBaseUrl = import.meta.env.VITE_API_URL || '';
  ...
  const [speedyBalance, setSpeedyBalance] = useState<{ indexer?: number; checker?: number } | null>(null);
  useEffect(() => {
    const fetchSpeedy = async () => {
      try {
        ...
        const backendBaseUrl = import.meta.env.VITE_API_URL || '';
        const res = await fetch(`${backendBaseUrl}/api/proxy/speedyindex/account`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data?.message || 'Failed to fetch');
        const indexer = data?.balance?.indexer ?? data?.result?.balance?.indexer;
        const checker = data?.balance?.checker ?? data?.result?.balance?.checker;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Select value={totalUsersFilter} onValueChange={handleTotalUsersFilterChange}>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(totalUsersData !== undefined ? totalUsersData : dashboardData?.totalUsers || 0).toLocaleString()}
            </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Select value={subscribedUsersFilter} onValueChange={handleSubscribedUsersFilterChange}>
              <SelectTrigger className="w-32 h-6 text-xs">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(subscribedUsersData !== undefined ? subscribedUsersData : dashboardData?.totalSubscribedUsers || 0).toLocaleString()}
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Select value={totalRevenueFilter} onValueChange={handleTotalRevenueFilterChange}>
              <SelectTrigger className="w-32 h-6 text-xs">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ${(totalRevenueData !== undefined ? totalRevenueData : dashboardData?.totalRevenue || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
            <p className="text-xs text-muted-foreground">
              Lifetime revenue
            </p>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Select value={conversionRateFilter} onValueChange={handleConversionRateFilterChange}>
              <SelectTrigger className="w-32 h-6 text-xs">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(() => {
                const totalUsers = totalUsersData !== undefined ? totalUsersData : dashboardData?.totalUsers || 0;
                const subscribedUsers = subscribedUsersData !== undefined ? subscribedUsersData : dashboardData?.totalSubscribedUsers || 0;
                return totalUsers > 0 ? ((subscribedUsers / totalUsers) * 100).toFixed(1) : '0';
              })()}%
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <div className="flex items-center space-x-2">
              <DollarSign className="h-4 w-4 text-muted-foreground" />
              <CardTitle className="text-sm font-medium">SpeedyIndex Balance</CardTitle>
            </div>
          </CardHeader>
          <CardContent>
            {speedyLoading ? (
              <div className="flex items-center text-sm text-gray-600"><Loader2 className="h-4 w-4 mr-2 animate-spin" />Loading…</div>
            ) : speedyError ? (
              <div className="text-sm text-red-600">{speedyError}</div>
            ) : (
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground">Indexer</div>
                <div className="text-xl font-semibold">{speedyBalance?.indexer ?? '-'}</div>
                <div className="text-xs text-muted-foreground mt-2">Checker</div>
                <div className="text-xl font-semibold">{speedyBalance?.checker ?? '-'}</div>
              </div>
            )}
          </CardContent>
        </Card>
