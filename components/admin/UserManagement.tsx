...
import { Search, Plus, Users, Calendar, ChevronLeft, ChevronRight, Activity, CreditCard, Info, Edit, Archive, Trash2, ArchiveRestore, UserCheck, Shield } from 'lucide-react';
import { AddCreditsDialog } from './AddCreditsDialog';
import { UserEditDialog } from './UserEditDialog';
import { UserData, UserTotalCredits } from '@/types/admin';
import { useAdminUsers, useArchiveUser, useUnarchiveUser, useDeleteUser } from '@/hooks/useAdminUsers';
...
export const UserManagement = () => {
  ...
  const [loadingBonusCredits, setLoadingBonusCredits] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    if (selectedUser && users.length > 0) {
      ...
      if (updatedUser && JSON.stringify(updatedUser) !== JSON.stringify(selectedUser)) {
        console.log('UserManagement - Updating selectedUser with fresh data:', updatedUser);
  ...
  const fetchBonusCredits = async (userId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${backendBaseUrl}/api/admin/users/${userId}/bonus-credits`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.code === 0) {
          setBonusCredits(prev => ({ ...prev, [userId]: result.result.bonusCredits || [] }));
        }
      }
    } catch (error) {
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {stats?.paidSubscriptions || 0} / {stats?.freeSubscriptions || 0}
                  </p>
                  ...
                  <TableBody>
                    {isLoading ? (
                      ...
                      users.map((user) => {
                        ...
                        const planName = getPlanDisplayName(user.subscription_plan_name, user.subscription_status);
                          ...
                          <TableRow key={user.user_id} className="hover:bg-muted/30 transition-colors">
                            <TableCell>
                              <div className="flex flex-col">
                                ...
                                {user.contactNumber && (
                                  <span className="text-xs text-muted-foreground">{user.contactNumber}</span>
                                )}
                                {user.isAdmin && (
                            ...
                            <TableCell>
                              <Badge className={`${getUserStatusBadgeColor(user.status || 'inactive')} border-0`}>
                            ...
                            <TableCell>
                              {remainingDays !== null ? (
                                <div className="flex items-center space-x-1">
                                  ...
                                  <span className={`font-medium ${remainingDays <= 7 ? 'text-red-600' : remainingDays <= 30 ? 'text-yellow-600' : 'text-green-600'}`}>
                                    {remainingDays} days
                                  </span>
                                </div>
                              ) : (
                            ...
                            <TableCell>
                              <div className="space-y-1">
                                <div className="font-medium">
                                  Total: {(user.total_credits || 0).toLocaleString()}
                                </div>
                                <div className="text-xs text-muted-foreground">
                                  Plan: {(user.subscription_credits || 0).toLocaleString()} | 
                                  <TooltipProvider>
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <span 
                                          className="cursor-help inline-flex items-center space-x-1"
                                          onMouseEnter={() => fetchBonusCredits(user.user_id)}
                                        >
                                          <span>Extra: {(user.extra_credits || 0).toLocaleString()}</span>
                                      ...
                                      <TooltipContent className="max-w-xs">
                                        ...
                                        ) : bonusCredits[user.user_id]?.length > 0 ? (
                                          <div className="space-y-2">
                                            <div className="font-medium text-sm">Bonus Credits:</div>
                                            {bonusCredits[user.user_id].map((credit, index) => (
                                              <div key={credit.id || index} className="text-xs space-y-1">
                                  ...
                                  <TooltipTrigger asChild>
                                    <div className="cursor-help">
                                      <div className="text-sm font-medium text-gray-900 ">
                                        {((user.total_used || 0) + (user.held_credits || 0)).toLocaleString()}
                                      </div>
                                      <div className="text-xs text-muted-foreground">
                                        Total Usage
                                      </div>
                                      ...
                                      <div className="space-y-1">
                                        <div className="flex justify-between text-xs">
                                          ...
                                          <span className="font-medium text-green-600 ">
                                            {(user.total_used || 0).toLocaleString()}
                                          </span>
                                        </div>
                                        <div className="flex justify-between text-xs">
                                          <span className="text-muted-foreground">Held Credits:</span>
                                          <span className="font-medium text-amber-600 ">
                                            {(user.held_credits || 0).toLocaleString()}
                                          </span>
                                        </div>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Are you sure you want to archive this user? The user will not be able to login until unarchived.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                                      <AlertDialogAction onClick={() => handleArchiveUser(user)}>Archive</AlertDialogAction>
                                    </AlertDialogFooter>
                                  </AlertDialogContent>
                                </AlertDialog>
                              </div>
                  ...
                  users.map((user) => {
                    ...
                    return (
                      <TableRow key={user.user_id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <TableCell>
                              <Badge className={`${getStatusBadgeColor(user.subscription_status || 'inactive')} border-0`}>
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Are you sure you want to unarchive this user? The user will be able to login again.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                                      <AlertDialogAction onClick={() => handleUnarchiveUser(user)} className="bg-green-600 hover:bg-green-700">
                                        Unarchive
                                      </AlertDialogAction>
                                    </AlertDialogFooter>
                                  </AlertDialogContent>
        ...
        {!isLoading && users.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">
              {searchTerm ? 'Try adjusting your search term.' : 'No users available.'}
            </p>
          </div>
        )}
