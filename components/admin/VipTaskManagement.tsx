...
const VipTaskManagement = () => {
  ...
  const { data: taskStats, isLoading: statsLoading, refetch: refetchStats } = useQuery({
    queryKey: ['vip-task-stats'],
    queryFn: async (): Promise<VipTaskStats> => {
      const backendBaseUrl = import.meta.env.VITE_API_URL || '';
      const response = await fetch(`${backendBaseUrl}/api/admin/vip-tasks/stats`, {
        headers: {
  ...
  const { data: queueStats, isLoading: queueLoading, refetch: refetchQueue } = useQuery({
    ...
    queryFn: async (): Promise<VipQueueStats> => {
      ...
      return {
        ...
        pending: data.stats.pending ?? (data.stats.waiting + (data.stats.delayed ?? 0)),
        active: data.stats.active,
        completed: data.stats.completed,
        failed: data.stats.failed,
        totalJobs: data.stats.waiting + (data.stats.delayed ?? 0) + data.stats.active + data.stats.completed + data.stats.failed,
      };
    },
  ...
  // Fetch delayed jobs
  const { data: delayedJobs, isLoading: delayedLoading, refetch: refetchDelayed } = useQuery({
  ...
  const { data: healthStatus, isLoading: healthLoading, refetch: refetchHealth } = useQuery({
    ...
    queryFn: async (): Promise<HealthStatus> => {
      const backendBaseUrl = import.meta.env.VITE_API_URL || '';
      const response = await fetch(`${backendBaseUrl}/api/vip/health/speedyindex`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const bulkRetryMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Bulk retry initiated: ${data.totalJobsRetried} jobs retried`);
  ...
  const retryTaskMutation = useMutation({
    mutationFn: async (taskId: string) => {
      const backendBaseUrl = import.meta.env.VITE_API_URL || '';
      const response = await fetch(`${backendBaseUrl}/api/vip/retry/${taskId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
    ...
    onSuccess: (data, taskId) => {
      toast.success(`Task ${taskId} retry initiated: ${data.totalJobsRetried} jobs retried`);
      queryClient.invalidateQueries({ queryKey: ['vip-failed-tasks'] });
      queryClient.invalidateQueries({ queryKey: ['vip-queue-stats'] });
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">VIP Task Management</h1>
          <p className="text-gray-600 mt-1">Monitor and manage VIP task processing and retry failed jobs</p>
        </div>
        <div className="flex items-center space-x-2">
          <Button
            onClick={() => {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-red-600">{taskStats?.failedTasks || 0}</div>
                <p className="text-xs text-muted-foreground">
                  Tasks with errors
                </p>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-blue-600">
                  {taskStats?.successRate ? `${taskStats.successRate.toFixed(1)}%` : '0%'}
                </div>
                <p className="text-xs text-muted-foreground">
                  Completion rate
                </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">{taskStats?.pendingTasks || 0}</Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">Processing</span>
                  <Badge variant="outline">{taskStats?.processingTasks || 0}</Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">Average Processing Time</span>
                  <span className="text-sm text-muted-foreground">
                    {taskStats?.averageProcessingTime ? `${taskStats.averageProcessingTime.toFixed(1)} min` : 'N/A'}
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">Pending (Waiting + Delayed)</span>
                  <Badge variant="outline">{queueStats?.pending || (queueStats?.waiting || 0) + (queueStats?.delayed || 0)}</Badge>
                </div>
                <div className="flex items-center justify-between">
        ...
        <TabsContent value="failed" className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              disabled={bulkRetryMutation.isPending || !failedTasks || failedTasks.length === 0}
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {task.failedJobs.map((job) => (
                        <div key={job.id} className="border-l-4 border-red-500 pl-4 py-2">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="font-medium text-sm">{job.name}</p>
                              <p className="text-xs text-muted-foreground">
                                Failed at: {format(new Date(job.timestamp), 'MMM dd, yyyy HH:mm:ss')}
                              </p>
                            </div>
        ...
        <TabsContent value="delayed" className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              size="sm"
            >
              <RefreshCw className={`w-4 h-4 mr-2 ${delayedLoading ? 'animate-spin' : ''}`} />
              Refresh
            </Button>
          </div>
          ...
          {delayedLoading ? (
            ...
            <div className="space-y-4">
              {delayedJobs.map((job) => {
                const remainingMinutes = Math.floor(job.remainingTime / (1000 * 60));
                const remainingSeconds = Math.floor((job.remainingTime % (1000 * 60)) / 1000);
                const pendingMinutes = Math.floor(job.pendingTime / (1000 * 60));
                const pendingSeconds = Math.floor((job.pendingTime % (1000 * 60)) / 1000);
                        ...
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span>{format(new Date(job.timestamp), 'MMM dd, yyyy HH:mm:ss')}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Scheduled for:</span>
                              <span>{format(new Date(job.delayUntil), 'MMM dd, yyyy HH:mm:ss')}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Delay Duration:</span>
                              <span>{pendingMinutes > 0 ? `${pendingMinutes}m ${pendingSeconds}s` : `${pendingSeconds}s`}</span>
                            </div>
                            <div className="flex justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {queueStats?.totalJobs ? 
                      `${queueStats.completed}/${queueStats.totalJobs} (${((queueStats.completed / queueStats.totalJobs) * 100).toFixed(1)}%)` : 
                      'N/A'
                    }
                  </span>
                </div>
        ...
        <TabsContent value="health" className="space-y-6">
          <h2 className="text-xl font-semibold">SpeedyIndex API Health Monitor</h2>
          ...
          {healthLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-sm text-muted-foreground mt-2">
                    Last checked: {format(new Date(healthStatus.timestamp), 'MMM dd, yyyy HH:mm:ss')}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="flex items-center justify-between">
                    <Badge variant={healthStatus.dns.status === 'resolved' ? 'default' : 'destructive'}>
                      {healthStatus.dns.status}
                    </Badge>
                  </div>
                  {healthStatus.dns.error && (
                    <p className="text-sm text-red-600 mt-2">{healthStatus.dns.error}</p>
                  )}
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={healthStatus.api.status === 'reachable' ? 'default' : 'destructive'}>
                    ...
                    {healthStatus.api.responseTime && (
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm text-muted-foreground">{healthStatus.api.responseTime}</span>
                      </div>
                    )}
                    {healthStatus.api.error && (
                      <p className="text-sm text-red-600">{healthStatus.api.error}</p>
                    )}
                  </div>
                </CardContent>
              </Card>
