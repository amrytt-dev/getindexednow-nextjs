...
import { Eye, EyeOff, RefreshCw, Calendar, CheckCircle, ChevronDown, ChevronUp, Download } from 'lucide-react';
...
export const TaskCard = ({ task, isUpdating, onUpdateStatus, statusLabel }: TaskCardProps) => {
  ...
  const { canRefresh, getTimeUntilNextRefresh, handleRefreshWithRateLimit, getLastRefreshTime } = useRefreshRateLimit(task.id);
  ...
  const computedStatus = task.status || (task.is_completed ? 'completed' : 'processing');

  // Calculate progress percentage
  const progressPercentage = typeof task.size === 'number' && typeof task.processed_count === 'number' && task.size > 0
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
            <div className="flex items-center space-x-3">
              <span className="text-xl">{task.type.includes('indexer') ? '🚀' : '🔍'}</span>
              <h3 className=" text-[18px] font-medium text-[#202124] truncate">
                {task.title}
              </h3>
            </div>
            <Badge className={`rounded-full px-3 py-1 text-sm font-medium hover:bg-${getStatusColor(computeTaskStatus(task))} ${getStatusColor(computeTaskStatus(task))}`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[#f8f9fa] rounded-lg p-3">
              ...
              <div className="text-[16px] font-medium text-[#202124]">{task.size?.toLocaleString() || '0'}</div>
            </div>
            <div className="bg-[#f8f9fa] rounded-lg p-3">
              <div className="text-xs text-[#5f6368] mb-1">Processed</div>
              <div className="text-[16px] font-medium text-[#4285f4]">{task.processed_count?.toLocaleString() || '0'}</div>
            </div>
            <div className="bg-[#f8f9fa] rounded-lg p-3">
              <div className="text-xs text-[#5f6368] mb-1">Indexed</div>
              <div className="text-[16px] font-medium text-[#34a853]">
                {task.indexed_count?.toLocaleString() || '0'}/{task.size?.toLocaleString() || '0'}
              </div>
            </div>
            <div className="bg-[#f8f9fa] rounded-lg p-3">
          ...
          <div className="flex items-center justify-between pt-3 border-t border-[#dadce0]">
            ...
            <div className="flex items-center space-x-2">
              {/* VIP sync button hidden by request */}
                             ...
                             {isCompleted && (task.type === 'indexer' || task.type === 'google/indexer' || task.type === 'fastindex/indexer') && (
                <Button 
                  variant="outline" 
                  size="sm"
          ...
          <Collapsible open={open} onOpenChange={setOpen}>
            <CollapsibleContent>
              <div className="mt-4 pt-4 border-t border-[#dadce0]">
                <TaskLinksTable
                    taskId={task.id}
                    type={task.type === 'checker' || task.type === 'google/checker' || task.type === 'fastindex/indexer' ? 'checker' : 'indexer'}
