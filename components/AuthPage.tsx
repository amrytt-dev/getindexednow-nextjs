...
import { Shield, LoaderCircle, ArrowLeft, LockKeyhole, Mail, CheckCircle, XCircle, Eye, EyeOff } from 'lucide-react';
import { motion } from 'framer-motion';
import { useRecaptcha } from '@/hooks/useRecaptcha';
import { useEmailAvailability } from '@/hooks/useEmailAvailability';
import { checkEnvironmentVariables, testEnvironmentVariables } from '@/utils/envChecker';
...
export const AuthPage = () => {
  ...
  const { renderRecaptcha, getRecaptchaResponse, resetRecaptcha, isLoaded, widgetRendered, retryRender } = useRecaptcha();

  // Debug: Log reCAPTCHA configuration
  useEffect(() => {
    console.log('AuthPage Debug - reCAPTCHA config:', {
      siteKey: import.meta.env.VITE_RECAPTCHA_SITE_KEY ? 'SET' : 'NOT SET',
      siteKeyLength: import.meta.env.VITE_RECAPTCHA_SITE_KEY?.length || 0,
      isLoaded,
      widgetRendered,
    ...
    if (isLoaded) {
      const renderWidget = async () => {
        try {
          ...
          const targetContainer = activeTab === 'signin' ? 'recaptcha-signin' : 'recaptcha-signup';
          console.log(`Attempting to render reCAPTCHA in container: ${targetContainer}`);
          ...
          setTimeout(async () => {
            try {
              const targetContainer = activeTab === 'signin' ? 'recaptcha-signin' : 'recaptcha-signup';
              console.log(`Retrying to render reCAPTCHA in container: ${targetContainer}`);
              await renderRecaptcha(targetContainer);
            } catch (retryError) {
              console.error('Retry failed to render reCAPTCHA widget:', retryError);
    ...
    if (selectedPlanData) {
      try {
        ...
        if (timeDiff < 30 * 60 * 1000) {
          ...
          // Small delay to ensure user context has time to update
          setTimeout(() => {
            navigate(`/plans-billing?selectedPlan=${planData.planId}`, { replace: true });
          }, 100);
          return;
        } else {
  ...
  const signUpForm = useForm<AuthFormValues>({
    defaultValues: { email: '', password: '', firstName: '', lastName: '', contactNumber: '', confirmPassword: '' },
    mode: 'onTouched',
  });
  ...
  const handleSignUp = async (values: AuthFormValues) => {
    ...
    console.log('SignUp - reCAPTCHA response length:', recaptchaResponse.length);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error?.["_errors"]?.[0] || data.error || 'Registration failed');
      }
      await setToken(data.token);
      toast({
        title: 'Account created',
        description: 'You have successfully registered. Please check your email to verify your account before logging in',
      });
  ...
  const handleSignIn = async (values: AuthFormValues) => {
    ...
    // Check reCAPTCHA response
    const recaptchaResponse = getRecaptchaResponse();
    console.log('SignIn - reCAPTCHA response length:', recaptchaResponse.length);
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error?.["_errors"]?.[0] || data.error || 'Login failed');
      }
  ...
  useEffect(() => {
    ...
    if (import.meta.env.VITE_DISABLE_GOOGLE_ONE_TAP === 'true') {
          ...
          if (window.google?.accounts?.id) {
            window.google.accounts.id.initialize({
              ...
              callback: async (response: any) => {
                ...
                try {
                  const res = await fetch(`${backendBaseUrl}/api/auth/google/onetap`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
      ...
      <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gradient-to-br from-primary/5 to-secondary/5 relative overflow-hidden">
        ...
        >
           <div className="w-full flex justify-center">
                <Link to="/"><img src="/logo.svg" alt="GetIndexedNow Logo" className="h-12 mb-4 lg:mb-6" /></Link>
              </div>
              ...
              {adminMessage && (
               ...
               <Tabs defaultValue="signin" className="w-full" onValueChange={setActiveTab}>
                 ...
                 <TabsContent value="signin">
                  ...
                  >
                    <svg className="h-5 w-5" viewBox="0 0 48 48"><g><path fill="#4285F4" d="M24 9.5c3.54 0 6.7 1.22 9.19 3.22l6.85-6.85C36.68 2.36 30.77 0 24 0 14.82 0 6.71 5.06 2.69 12.44l7.98 6.2C12.13 13.13 17.62 9.5 24 9.5z"/><path fill="#34A853" d="M46.1 24.55c0-1.64-.15-3.22-.42-4.74H24v9.01h12.42c-.54 2.9-2.18 5.36-4.65 7.02l7.19 5.59C43.93 37.36 46.1 31.47 46.1 24.55z"/><path fill="#FBBC05" d="M10.67 28.64c-1.01-2.99-1.01-6.29 0-9.28l-7.98-6.2C.64 17.1 0 20.47 0 24c0 3.53.64 6.9 1.77 10.13l7.98-6.2z"/><path fill="#EA4335" d="M24 48c6.48 0 11.92-2.14 15.89-5.82l-7.19-5.59c-2.01 1.35-4.59 2.16-8.7 2.16-6.38 0-11.87-3.63-14.33-8.94l-7.98 6.2C6.71 42.94 14.82 48 24 48z"/><path fill="none" d="M0 0h48v48H0z"/></g></svg>
                    Continue with Google
                  </Button>

                  <div className="flex items-center my-6">
                    <div className="flex-1 h-px bg-border" />
                    <span className="mx-3 text-xs text-foreground/50">or continue with email</span>
                    <div className="flex-1 h-px bg-border" />
                  </div>
                    ...
                    <form onSubmit={signInForm.handleSubmit(handleSignIn)} className="space-y-4">
                      ...
                      <FormField
                       ...
                       <div className="flex flex-col items-center mb-4">
                         <div id="recaptcha-signin" onClick={handleRecaptchaInteraction}></div>
                         {recaptchaError && activeTab === 'signin' && (
                          ...
                          {isLoaded && !widgetRendered && activeTab === 'signin' && (
                            <div className="text-center">
                ...
                <TabsContent value="signup">
                  ...
                  <div className="flex items-center my-6">
                    <div className="flex-1 h-px bg-border" />
                    <span className="mx-3 text-xs text-foreground/50">or register with email</span>
                    <div className="flex-1 h-px bg-border" />
                  </div>
                                ...
                                <FormControl>
                                  <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        autoComplete="email"
                                        className={`pl-10 pr-10 ${
                                          emailAvailability.isAvailable === true ? 'border-green-500 focus:border-green-500' :
                                          emailAvailability.isAvailable === false ? 'border-red-500 focus:border-red-500' : ''
                                        }`}
                                        {...field}
                                    />
                                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                      ...
                                      {emailAvailability.isAvailable === true && !emailAvailability.isChecking && (
                                        <CheckCircle className="h-4 w-4 text-green-500" />
                                      )}
                                      {emailAvailability.isAvailable === false && !emailAvailability.isChecking && (
                          ...
                          render={({ field }) => (
                              <FormItem>
                                ...
                                <FormMessage />
                                             ...
                                             />
                      <div className="flex flex-col items-center mb-4">
                        <div id="recaptcha-signup" onClick={handleRecaptchaInteraction}></div>
                        ...
                        {isLoaded && !widgetRendered && activeTab === 'signup' && (
                          <div className="text-center">
                            <p className="text-sm text-blue-600 mt-2 font-medium bg-blue-50 px-3 py-2 rounded-md border border-blue-200">
                              Initializing reCAPTCHA...
                            </p>
                       ...
                       <Button
                          ...
                          size="lg"
                          variant="hero"
                          disabled={loading || emailAvailability.isAvailable === false || emailAvailability.isChecking}
                      >
                        {loading ? (
                            <span className="flex items-center">
                          <LoaderCircle className="mr-2 h-4 w-4 animate-spin" />
          ...
          <p className="text-center mt-6 text-xs text-foreground/60">
            ...
            <Link to="/terms-and-conditions" className="text-primary hover:underline mx-1">Terms of Service</Link>
            and
