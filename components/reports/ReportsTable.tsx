...
export const ReportsTable = ({ tasks, onRefresh }: ReportsTableProps) => {
  ...
  const handleViewReport = async (task: Task) => {
    ...
    try {
      toast({
        ...
        description: "Please implement your own backend API for viewing reports.",
      });
    } catch (error) {
      toast({
  ...
  const handleDownloadCsv = async (taskId: string, event: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/reports/download?format=csv&taskId=${taskId}`, {
        method: 'GET',
        headers: {
  ...
  const getReportStatusBadge = (task: Task) => {
    ...
    if (status === 'available') {
      return <Badge variant="default" className="bg-green-500 text-white">Report available</Badge>;
    } else if (status === 'not_ready') {
      return <Badge variant="destructive" className="bg-red-500 text-white">Report not generated yet</Badge>;
    } else {
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div 
                        ...
                        onClick={() => navigate(`/tasks?taskId=${task.id}&type=${task?.type?.split('/').pop()}`)}
                      >
                        {task.title}
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>{typeof task.size === 'number' ? task.size : (task.link_count ?? '-')}</TableCell>
                  <TableCell>
                    ...
                    <div className="space-y-1">
                      ...
                      {task.report_last_checked && (
                        <div className="text-xs text-muted-foreground">
                          Last checked: {format(new Date(task.report_last_checked), 'MMM dd, HH:mm')}
                        </div>
                      )}
                    </div>
                  </TableCell>
