...
export const useRecaptcha = () => {
  ...
  useEffect(() => {
    ...
    if (!window.grecaptcha && !scriptLoadedRef.current) {
      if (!siteKey) {
        ...
        console.error('Please set VITE_RECAPTCHA_SITE_KEY in your environment variables');
        return;
      }
          ...
          try {
            ...
            const widgetId = window.grecaptcha.render(containerId, {
              ...
              callback: (response: string) => {
                console.log(`reCAPTCHA v2 callback triggered for ${containerId}, response length:`, response.length);
            ...
            setRenderAttempts(0);
            console.log(`reCAPTCHA v2 widget rendered with ID: ${widgetId} in ${containerId}`);
            resolve(widgetId);
          } catch (error) {
  ...
  const getRecaptchaResponse = useCallback((): string => {
    ...
    if (globalResponse) {
      console.log('Getting reCAPTCHA response from global storage, length:', globalResponse.length);
    ...
    try {
      ...
      console.log('Getting reCAPTCHA response from widget, length:', response.length);
      return response;
    } catch (error) {
  ...
  return { renderRecaptcha, getRecaptchaResponse, resetRecaptcha, isLoaded, widgetRendered, retryRender };
}; 