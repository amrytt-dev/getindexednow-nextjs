...
export const useDashboardData = () => {
  const [subscription, setSubscription] = useState<CustomUserSubscription | null>(null);
  const [creditUsage, setCreditUsage] = useState<CreditUsage>({ indexer_used: 0, checker_used: 0, total_used: 0 });
  ...
  const fetchSubscription = async () => {
    ...
    try {
      ...
      const responseData = await getWithAuth<{ hasActiveSubscription: boolean; subscription: CustomUserSubscription }>('/user/subscription');
      ...
      if (responseData.hasActiveSubscription && responseData.subscription) {
        ...
        const freeSubscription: CustomUserSubscription = {
          ...
          subscription_plans: {
            name: 'Free',
            monthly_credits: parseInt(import.meta.env.FREE_CREDIT || '4'),
            price_monthly: 0,
      ...
      console.log('Dashboard: fetchSubscription completed, setting loading to false');
