...
export const useInvoices = () => {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const data: InvoiceResponse = await getWithAuth<InvoiceResponse>('/invoices');
  ...
  const downloadInvoice = async (invoiceId: string): Promise<boolean> => {
    try {
      ...
      console.log('📡 Making request to:', `${import.meta.env.VITE_API_URL}/api/invoices/${invoiceId}/download`);

      // Create a timeout promise
      const timeoutPromise = new Promise<Response>((_, reject) => {
        setTimeout(() => reject(new Error('Download timeout - request took too long')), 30000); // 30 seconds
      });

      // Create the download promise
      const downloadPromise = downloadWithAuth(`/invoices/${invoiceId}/download`);
...
export const useInvoiceDetails = (invoiceId: string | null) => {
  ...
  const fetchInvoiceDetails = async () => {
    ...
    try {
      ...
      const data: InvoiceResponse = await getWithAuth<InvoiceResponse>(`/invoices/${invoiceId}`);
