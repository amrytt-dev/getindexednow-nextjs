...
export default function Plans() {
  ...
  const handleRevokeSubscription = async () => {
    ...
    if (
      !confirm("Are you sure you want to revoke your current subscription?")
    ) {
      return;
    }

    try {
      const data = await deleteWithAuth(
        `/admin/subscriptions/${currentSubscription.id}`
      );
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  ${(stats.totalMonthlyRevenue / 100).toFixed(2)}
                </p>
              </div>
            </div>
          </CardContent>
                  ...
                  >
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <Badge
                            className={`${getPlanColor(plan.name)} border-0`}
                          >
                            {plan.name}
                          </Badge>
                          <Badge
                        ...
                        <div className="flex space-x-1">
                          ...
                          <Button
                            ...
                            disabled={
                              plan.activeUsers > 0 || deleting === plan.id
                            }
                      ...
                      {plan.restrictions && (
                        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                          ...
                          <ul className="text-xs text-yellow-700 mt-1 space-y-1">
                            {plan.restrictions.price && (
                              <li>• {plan.restrictions.price}</li>
                            )}
                            {plan.restrictions.credits && (
                              <li>• {plan.restrictions.credits}</li>
                            )}
                          </ul>
                        </div>
                      )}
                      ...
                      {!plan.isActive && plan.activeUsers > 0 && (
                        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                          ...
                          <p className="text-xs text-blue-700 mt-1">
                            This plan is marked as legacy but still available
                            for new subscriptions. Has {plan.activeUsers} active
                            user(s).
                          </p>
                        </div>
                      )}
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {currentSubscription.plan.descriptions}
                      </p>
                    </div>
                    <Badge
                      variant={
                        currentSubscription.isActive ? "default" : "secondary"
                      }
                    >
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      {currentSubscription.plan.credits.toLocaleString()}
                    </div>
                    <div>
                ...
                <div className="text-center py-8">
                  ...
                  >
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        <div>
                          <Label htmlFor="admin-user-select">
                            Select Admin User
                          </Label>
                          <Select
                        ...
                        {selectedAdminUserId && (
                          <>
                            <p className="text-sm text-gray-600">
                              Select an admin plan to assign:
                            </p>
                            <div className="space-y-2">
                              {adminPlans.map((plan) => (
                                <div
                        ...
                        {!selectedAdminUserId && (
                          <p className="text-sm text-gray-500">
                            Please select an admin user first to see available
                            plans.
                          </p>
                        )}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Button
                  onClick={() => setShowAdminCreateDialog(true)}
                  className="flex items-center space-x-2"
                >
                  <Plus className="h-4 w-4" />
            ...
            {editingPlan && editingPlan.restrictions && (
              <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                ...
                <p className="text-xs text-yellow-700 mt-1">
                  Price and credits cannot be modified for plans with active
                  subscriptions.
                </p>
              </div>
            )}
      ...
      <Dialog
        open={showAdminCreateDialog}
        onOpenChange={setShowAdminCreateDialog}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={adminFormData.billingCycle}
                onValueChange={(value) =>
                  setAdminFormData({ ...adminFormData, billingCycle: value })
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            <div className="flex justify-end space-x-2">
              <Button
                variant="outline"
                onClick={() => setShowAdminCreateDialog(false)}
              >
                Cancel
              </Button>
              <Button onClick={handleCreateAdminPlan}>Create Plan</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
