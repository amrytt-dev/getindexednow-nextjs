...
export default function Faqs() {
  ...
  const categories = Array.from(
    new Set(
      faqs.map((f: any) => f.category?.toLowerCase().trim()).filter(Boolean)
    )
  ).map(
    (cat) =>
      faqs.find((f: any) => f.category?.toLowerCase().trim() === cat)
        ?.category || cat
  );
  ...
  const onSubmit = async (formData: FaqFormData) => {
    try {
      ...
      toast({
        title: "Error",
        description: e.message || "Failed to save FAQ.",
      });
    }
  };
  ...
  const handleDelete = async (faqId: string) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: e.message || "Failed to delete FAQ.",
      });
    }
  };
