...
export const checkEnvironmentVariables = () => {
  const envVars = {
    ...
    VITE_GOOGLE_CLIENT_ID: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID,
  ...
  console.log(
    "reCAPTCHA Site Key:",
    envVars.VITE_RECAPTCHA_SITE_KEY ? "SET" : "NOT SET"
  );
  console.log(
    "Google Client ID:",
    envVars.VITE_GOOGLE_CLIENT_ID ? "SET" : "NOT SET"
  );
  ...
  if (envVars.VITE_GOOGLE_CLIENT_ID) {
    ...
    console.warn(
      "⚠️ Google Client ID is not set (Google Sign-In will be disabled)"
    );
  }
  ...
  return {
    recaptchaConfigured:
      !!envVars.VITE_RECAPTCHA_SITE_KEY &&
      envVars.VITE_RECAPTCHA_SITE_KEY !== "your_recaptcha_site_key_here",
    googleConfigured:
      !!envVars.VITE_GOOGLE_CLIENT_ID &&
      envVars.VITE_GOOGLE_CLIENT_ID !== "your_google_client_id_here",
